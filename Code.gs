...
function refreshProcurementMaster() {
  ...
  try {
    ...
    appendRunLog_(
      ...
      'Generated sheet headers created and reference sheets refreshed; no raw sheets were modified. Company refs: ' + referenceResult.companyCount + ', department refs: ' + referenceResult.departmentCount + '.',
      {sourceRowCount: referenceResult.sourceRowsScanned}
    );
  } finally {
    lock.releaseLock();
  }
...
function refreshReferences() {
  ...
  try {
    ...
    appendRunLog_(
      ...
      'refreshReferences',
      'success',
      'Reference sheets refreshed from LIST. Company refs: ' + referenceResult.companyCount + ', department refs: ' + referenceResult.departmentCount + '.',
      {sourceRowCount: referenceResult.sourceRowsScanned}
    );
  } finally {
    lock.releaseLock();
